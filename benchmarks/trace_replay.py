...
class TraceReplayDataset(BenchmarkDataset):
    ...
    def load_trace_file(self, trace_file):
        with open(trace_file, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                self.REG_GROUPS[timestamp].append(record)
                self.REG_RECORDS.append((timestamp, record))

    def load_trace(self, trace_paths):
        self.REG_GROUPS = defaultdict(list)
        self.REG_RECORDS = []
    ...
    def __init__(
        ...
        **kwargs,
        ...
        trace_paths = trace_path if isinstance(trace_path, list) else [trace_path]
        # Load trace file
        self.load_trace(trace_paths)
        trace_directory_name = os.path.dirname(trace_paths[0])
        trace_file_name = os.path.basename(trace_paths[0]).split(".")[0]
        if len(trace_paths) > 1:
            trace_file_name = f"{trace_file_name}_merged_{len(trace_paths)}"
    ...
    def sample(
        ...
        preserve_trace_order: bool = False,
        **kwargs,
        ...
        for index, (timestamp, req) in enumerate(record_items):
            request_time = float(index) if preserve_trace_order else timestamp
            hash_ids = req["hash_ids"]
            input_length = req["input_length"]
            output_length = req["output_length"]

            prompt = self.generate_prompt(hash_ids, input_length, tokenizer)
            requests[request_time].append(
                SampleRequest(
                    prompt=prompt,
...
def gene_prompts_by_dataset_name(
    ...
    tokenizer: PreTrainedTokenizerBase,
    args: argparse.Namespace,
    req_records: Optional[list] = None,
    preserve_trace_order: bool = False,
) -> dict[float, list]:
    ...
    for index, (sec, req) in enumerate(record_items):
        request_time = float(index) if preserve_trace_order else sec
        # Try to produce prompt by benchmark datasets
        gene_req = gene_one_req(req, tokenizer, args)
        input_requests[request_time].extend(gene_req)
...
async def replay_trace_by_time(
    ...
    flat_requests = [
        req for _, reqs in sorted(req_groups.items()) for req in reqs
    ]
    outputs = []
    ...
    if getattr(args, "ignore_timestamp", False):
        print("Ignoring trace timestamps; sending requests sequentially.")
        for index, sample_req in enumerate(flat_requests, start=1):
            try:
                outputs.append(await _run_one_request(sample_req))
            except asyncio.TimeoutError:
                print(f"Request timed out: sequential request {index}")
            except Exception as e:
                print(f"Request failed: sequential request {index}: {e}")
    else:
        tasks = []
...
def create_argument_trace():
    ...
    trace_group.add_argument(
        ...
        nargs="+",
        default=None,
        help="Path to one or more trace jsonl files.",
    )
    trace_group.add_argument(
        "--trace-mode",
        type=str,
    ...
    trace_group.add_argument(
        "--ignore-timestamp",
        action="store_true",
        help="Ignore trace timestamps and send requests sequentially in trace order.",
    )
    return parser
...
def main(args: argparse.Namespace):
    ...
    if args.trace_mode == "trace":
        input_requests = dataset.sample(
            ...
            preserve_trace_order=args.ignore_timestamp,
        )
    else:
        input_requests = gene_prompts_by_dataset_name(
            req_groups=dataset.REG_GROUPS,
            tokenizer=tokenizer,
            args=args,
            req_records=dataset.REG_RECORDS,
            preserve_trace_order=args.ignore_timestamp,
        )

    # Send prompts by the selected replay schedule

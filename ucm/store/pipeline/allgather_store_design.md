...
This design replaces the connector-level `UcmAllGatherStore` wrapper with a
native `StoreV1` stage inside `UcmPipelineStore`.
...
UcmPipelineStore
      |
AllGatherStore        owns block routing, fused device buffers, collectives,
...
The existing connector-level implementation remains unchanged as the baseline
until the new pipeline passes correctness and performance gates. The first
implementation does not delete or silently redirect `UcmAllGatherStore`.
...
- Store each replicated block on one hash owner and use HCCL/NCCL plus one fused
  scatter kernel to materialize the complete KV block set on every TP rank.
...
- Preserve bounded load/dump slots, FA/WA shared progress, prepared layerwise
  metadata, and asynchronous `Check()` behavior from the baseline.
...
The stage uses HCCL or NCCL through a native platform-runtime interface and a
dedicated communicator. It does not invoke a PyTorch process group from the
native progress thread. This avoids Python GIL
dependencies and prevents background cache collectives from interleaving with
model collectives on the vLLM TP communicator.
...
Pipeline detection replaces connector-name detection. Code must check the
parsed pipeline capabilities rather than
`ucm_connector_name == "UcmAllGatherStore"`.
...
## Communicator and shared progress runtime
...
`liballgatherstore.so` therefore owns a process-local runtime registry keyed by
`allgather_runtime_key`. A runtime contains:

- one HCCL or NCCL communicator for the TP group;
...
On the TP group's rank zero, a small native binding creates an HCCL or NCCL root-info
blob. The existing vLLM TP group broadcasts that blob during Store
construction. Each native AllGather stage passes the blob, rank, and world size
to the registry. The first stage initializes the communicator; later stages
reuse it. TP1 creates a progress runtime without a collective communicator.
...
Each stage allocates rank-local device buffers. CacheStore host memory is also
rank-private.

For fixed window size `W = 4`, aligned packed shard size `S`, TP size `P`, load
slots `L`, dump slots `D`, and replicated-mode flag `R`, one stage allocates:

```text
load send       L * W * S
load receive    L * P * W * S        R and P > 1 only
dump send       D * W * S

payload bytes   W * S * (L + D + (R and P > 1 ? L * P : 0))
```

TP1 aliases the receive buffer to the send buffer and therefore has no separate
receive allocation. Replicated TP ranks allocate the complete receive area
needed by the fixed-size AllGather. Non-replicated mode omits that area because
it does not execute a collective.
...
2. Acquire or reference prepared destination and route metadata.
...
5. The shared progress thread processes tasks in FIFO order. For each window:
   ...
   - enter the fixed-size HCCL/NCCL AllGather on every rank in identical order;
   - launch one compact-scatter kernel into the original tensor addresses;
...
AllGatherStore starts its collective only after the CacheStore backend task for
that window completes. Two or more load slots allow CacheStore work for later
windows to overlap with the collective and scatter for the current window.
...
## Prepared layerwise metadata

Rebuilding destination and route tables for every layer was measurable in the
baseline. The pipeline version retains the prepared-load optimization through
an optional native capability interface:

```text
PreparedLoadStoreV1
  PrepareLoadBatch(...)
  LoadPrepared(batch, request, shard_indices, layer_index)
  ReleasePreparedLoadBatch(batch)
```

`PipelineStore` exposes these methods only when its entry Store implements the
capability. `UcmPipelineStore` forwards them with the same Python method names
used by the current vLLM connector. The connector selects prepared loading by
capability, not by connector name.

A prepared batch owns reusable device address and route tables. Individual
tasks hold references. Releasing the Python batch prevents new tasks but does
not free its arena until all referenced tasks finish.
...
- prepared metadata allocation/reuse counts and bytes;
...
The kernel launcher is runtime-neutral and accepts raw device pointers and an
ACL stream. The current Torch binding may call the same launcher so the
connector-level baseline and pipeline implementation do not carry independent
kernel copies.
...
5. Add prepared-load capability forwarding to PipelineStore.
6. Change vLLM integration to inject runtime metadata based on pipeline
   capabilities while keeping `UcmAllGatherStore` operational.
7. Run direct Store A/B tests against baseline commit `bf69d67`.
8. Run DeepSeek V4 Flash end-to-end A/B tests on A2.
9. Deprecate the connector-level store only after functional and performance
   parity; remove it in a separate change.
...
- layerwise prepared metadata reuse and release during shutdown;

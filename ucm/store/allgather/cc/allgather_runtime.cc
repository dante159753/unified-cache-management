    ...
    bool collectiveEnabled, size_t loadGroupCount)
{
        ...
        if (auto runtime = found->second.lock()) {
            if (runtime->collectiveMode_ != collectiveMode) {
                return Status::InvalidParam(
                    "allgather runtime collective mode mismatch({}/{})",
                    collectiveMode, runtime->collectiveMode_);
            }
    ...
    auto status = runtime->Setup(rank, worldSize, collectiveBufferMb, collectiveMode,
                                 rootInfo,

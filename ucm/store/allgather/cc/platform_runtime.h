...
public:
    ...
    virtual Status EventElapsedTime(float* milliseconds, EventHandle start,
                                    EventHandle end) = 0;

    virtual Status CopyHostToDevice(void* destination, size_t destinationBytes,
                                    const void* source, size_t bytes,
                                    StreamHandle stream) = 0;

    virtual Expected<std::vector<uint8_t>> CreateRootInfo() = 0;
    virtual size_t RootInfoSize() const = 0;
    virtual Status CreateCollective(uint32_t rank, uint32_t worldSize,
                                    uint32_t bufferMb,
    ...
    virtual Status AllGather(void* send, void* receive, size_t bytes,
    ...
    virtual Status LaunchSegmentedCopy(StreamHandle stream, void* descriptors,
                                       void* coreOffsets, uint32_t usedWorkers) = 0;
    virtual Status LaunchCompactScatter(StreamHandle stream, void* receiveBuffer,
                                        void* destinationAddresses, void* routes,

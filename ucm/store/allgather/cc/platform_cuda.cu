...
{
    if (((reinterpret_cast<uintptr_t>(source) | reinterpret_cast<uintptr_t>(destination) |
          bytes) & (sizeof(uint4) - 1)) == 0) {
...
__global__ void SegmentedCopyKernel(const uint64_t* descriptors,
                                    const uint32_t* workerOffsets)
{
...
__global__ void CompactScatterKernel(const uint8_t* receiveBuffer,
                                     const uint64_t* destinationAddresses,
                                     const uint32_t* routes, const uint64_t* chunks,
                                     ...
                                     uint64_t shardSize)
{
    ...
    for (uint32_t task = blockIdx.x; task < taskCount; task += gridDim.x) {
        ...
        const auto* source = receiveBuffer + owner * rankStride + ownerSlot * shardSize +
                             chunks[chunkOffset + 1];
        auto* destination = reinterpret_cast<uint8_t*>(destinationAddress) +
                            chunks[chunkOffset + 2];
...
public:
    ...
    {
        return CudaStatus(cudaEventRecord(static_cast<cudaEvent_t>(event),
    ...
    {
        return CudaStatus(cudaStreamWaitEvent(static_cast<cudaStream_t>(stream),
    ...
    {
        return CudaStatus(cudaEventElapsedTime(milliseconds,
                            ...
                            CollectiveHandle* collective) override
    {
        ...
        return NcclStatus(ncclCommInitRank(reinterpret_cast<ncclComm_t*>(collective),
                                           worldSize, id, rank),
                          "ncclCommInitRank");
    }
    ...
    {
        return NcclStatus(ncclAllGather(send, receive, bytes, ncclInt8,
                                        static_cast<ncclComm_t>(collective),
                                        static_cast<cudaStream_t>(stream)),
                          "ncclAllGather");
    }
                               ...
                               void* coreOffsets, uint32_t usedWorkers) override
    {
        SegmentedCopyKernel<<<usedWorkers, 256, 0, static_cast<cudaStream_t>(stream)>>>(
            static_cast<const uint64_t*>(descriptors),
            static_cast<const uint32_t*>(coreOffsets));
                                ...
                                uint64_t shardSize, uint32_t usedWorkers) override
    {
        CompactScatterKernel<<<usedWorkers, 256, 0, static_cast<cudaStream_t>(stream)>>>(
            ...
            static_cast<const uint32_t*>(routes), static_cast<const uint64_t*>(chunks),
